...
public class JavaTimeTagLibraryValidator extends TagLibraryValidator {
	...
	/*
	 * Expression syntax validation has been disabled since when I ported this code
	 * over from Jakarta Taglib, I wanted to reduce dependencies. As I understand
	 * it, JSP 2.0 containers take over the responsibility of handling EL code (both
	 * in attribute tags, and externally), so this shouldn't be a problem unless
	 * you're using something old. If you want to restore this validation, you must
	 * uncomment the various lines in this source, include the Jakarta Taglib's
	 * standard.jar library at build and runtime, and (I believe, but don't know
	 * specifically) make a legacy-style tld which describes which attributes should
	 * be validated. Have a look at fmt.tld, fmt-1.0.tld, fmt-1.0-rt.tld in
	 * standard.jar for an example of this.
	 */

	// *********************************************************************
	// Implementation Overview
	/*
	 * We essentially just run the page through a SAX parser, handling the callbacks
	 * that interest us. We collapse <jsp:text> elements into the text they contain,
	 * since this simplifies processing somewhat. Even a quick glance at the
	 * implementation shows its necessary, tree-oriented nature: multiple Stacks, an
	 * understanding of 'depth', and so on all are important as we recover necessary
	 * state upon each callback. This TLV demonstrates various techniques, from the
	 * general "how do I use a SAX parser for a TLV?" to "how do I read my init
	 * parameters and then validate?" But also, the specific SAX methodology was
	 * kept as general as possible to allow for experimentation and flexibility.
	 */
...
//        config = null;
	...
	public synchronized ValidationMessage[] validate(final String prefix, final String uri, final PageData page) {
		try {
			this.uri = uri;
			// initialize
	...
	private boolean isTag(final String tagUri, final String tagLn, final String matchUri, final String matchLn) {
...
//    }
	...
	private boolean isJavaTimeTag(final String tagUri, final String tagLn, final String target) {
		return isTag(tagUri, tagLn, this.uri, target);
	}

	// utility method to determine if an attribute exists
	private boolean hasAttribute(final Attributes a, final String att) {
...
//    }
	...
	/**
	 * @param a used to get the 'scope' attribute
	 * @return true if the 'scope' attribute is valid
	 */
	protected boolean hasNoInvalidScope(final Attributes a) {
		final String scope = a.getValue(SCOPE);
		return !((scope != null) && !scope.equals(PAGE_SCOPE) && !scope.equals(REQUEST_SCOPE)
				&& !scope.equals(SESSION_SCOPE) && !scope.equals(APPLICATION_SCOPE));
	}

	/**
	 * @param a used to get the 'var' attribute
	 * @return true if the 'var' attribute is empty
	 */
	...
	/**
	 * @param a used to get the 'scope' attribute
	 * @return true if the 'scope' attribute is present without 'var'
	 */
	...
	/**
	 * @param qname the QName to get the local part from
	 * @return local part of a QName
	 */
	protected static String getLocalPart(final String qname) {
	...
	private class Handler extends DefaultHandler {
		...
		public void startElement(final String ns, String ln, final String qn, final Attributes a) {
			...
			if (qn.startsWith(prefix + ":") && !isJavaTimeTag(ns, ln, SET_ZONEID) && hasDanglingScope(a)) {
